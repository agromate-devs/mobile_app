<script lang="ts">
	...
	import { current_page, firebase_app } from '../lib/states.svelte';
	...
	onMount(() => {
		firebase_app.app = initializeApp(firebaseConfig);
		f7ready(() => {
		...
		{#if current_page.name != 'home'}
			<Toolbar tabbar icons position="bottom">
				<Link
					...
					onclick={() => change_tab('/homepage/', 'tab-1')}
				/>
				<Link
					id="tab-2"
					tabLink="#tab-2"
					...
					iconIos="f7:star"
					iconMd="f7:star"
					onclick={() => change_tab('/wishlist/', 'tab-2')}
				/>
				<Link
					id="tab-3"
					tabLink="#tab-3"
					...
					onclick={() => change_tab('/profile/', 'tab-3')}
				/>
			</Toolbar>
		{/if}
	</View>
