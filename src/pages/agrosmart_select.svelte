<script lang="ts">
	...
	async function assign_plant(device_id) {
		...
		const request_body = JSON.stringify({
			...
			default_temperature: plant_settings.temperature,
			temperature_limit: 20.5, // Useless, will drop soon
			notify_wrong_temperature: true,
			default_humidity: plant_settings.umidita.value,
			humidity_limit: 20.5, // Useless, will drop soon
			notify_wrong_humidity: plant_settings.umidita.limit,
			default_precipitation: plant_settings.precipitazioni.value,
			precipitation_limit: 20.5, // Useless, will drop soon
			notify_wrong_soil_humidity: plant_settings.precipitazioni.terreno_arido,
			default_light_color: plant_settings.illuminazione.wheePickerValue.hex,
			light_time: 3, // $ora_fine - $ora_inizio
			light_intensity: plant_settings.illuminazione.value
		});
		...
		{#each info as net, i}
			<div class="grid grid-cols-2">
				...
				<div>
					...
					<Button fill onclick={() => assign_plant(net.device_id)}>Assegna pianta</Button>
				</div>
			</div>
		{/each}
	</Block>
