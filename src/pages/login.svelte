<script lang="ts">
	...
	import { firebase_app, latest_wishlist_change_device_uuid } from '../lib/states.svelte';
	import { v4 as uuidv4 } from 'uuid';
	import type { AccountInfo } from '../lib/models/account_info';
	...
	async function add_device_to_db(uid: string) {
		...
		const db = getFirestore(firebase_app.app);
		...
		if (docSnap.exists()) {
			...
			latest_wishlist_change_device_uuid.value = document.latest_wishlist_change_device_uuid;
...
<Page>
	...
	<List strongIos dividersIos insetIos>
		...
		<div class="float-right">
			<ListItem onclick={reset_pwd} title="Password dimenticata?" />
		</div>
	</List>
	<br />
	...
	<!-- <br /> -->
	<Block>
		<Button onclick={login} fill>Accedi</Button>
		<br />
