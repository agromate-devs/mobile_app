<script lang="ts">
	...
	async function save_plant_into_wishlist() {
		...
		latest_wishlist_change_device_uuid.value = localStorage.getItem('device_uuid');
		...
		if (docSnap.exists()) {
			...
			latest_wishlist_change_device_uuid.value = device_uuid;
...
<Page>
	<div class="rectangle" style="--image:url('{selected_plant_photo}')">
		...
		<div class="freccia">
			<a class="link" href="#0" onclick={() => go_back()}>
		...
		<div class="search_bar">
			<Fab position="right-top" color="red" onclick={save_plant_into_wishlist}>
