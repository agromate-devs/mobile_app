...
function is_query_empty(query: ImageQuery | DescriptionQuery) {
	...
	if (query.query.pages['-1'] != null)
		// If -1 object exists in query, the result should be empty
		return true;
...
export async function get_plant_photo(plant: string) {
	// Get photo from Wikipedia
	const query: ImageQuery = await (
		await fetch(
			WIKIPEDIA_ENDPOINT.concat(
				'action=query&titles='
					.concat(plant)
					.concat('&prop=pageimages&format=json&pithumbsize=500&redirects&origin=*')
			)
		)
	).json();

	if (is_query_empty(query)) return null;
...
export async function get_plant_description(plant: string) {
	const query: DescriptionQuery = await (
		await fetch(
			WIKIPEDIA_ENDPOINT.concat(
				'format=json&action=query&prop=extracts&exintro&explaintext&redirects&origin=*&titles='
			).concat(plant)
		)
	).json();
